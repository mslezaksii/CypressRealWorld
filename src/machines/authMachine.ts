  ...
  },
  {
    services: {
      performSignup: async (ctx, event) => {
        ...
        const resp = await httpClient.post(`http://104.248.31.189:${backendPort}/users`, payload);
        history.push("/signin");
        return resp.data;
      },
      performLogin: async (ctx, event) => {
        return await httpClient
          .post(`http://104.248.31.189:${backendPort}/login`, event)
          .then(({ data }) => {
            history.push("/");
            return data;
          })
      ...
      getUserProfile: async (ctx, event) => {
        const resp = await httpClient.get(`http://104.248.31.189:${backendPort}/checkAuth`);
        return resp.data;
      },
      ...
      updateProfile: async (ctx, event: any) => {
        ...
        const resp = await httpClient.patch(
          `http://104.248.31.189:${backendPort}/users/${payload.id}`,
          payload
        );
        return resp.data;
      },
      performLogout: async (ctx, event) => {
        ...
        return await httpClient.post(`http://104.248.31.189:${backendPort}/logout`);
      },
